...
//this is called head but it is the end pointer of the task linkedlist
struct task *head = NULL;
int TASK_NUM = 1;
pthread_mutex_t lock;


//task struct with a pointer to the next task
struct task
{
    struct bitmap *bm;
    double xmin;
...
//struct list that stores the head of the linked list
struct list{
  struct task *head;
};

//function that dynamically create a task
...
//function that pops the front of a linkedlist
void task_dequeue(struct list *l){
  l->head = l->head->next;
}
...
int value_to_color(double x, double y, double complex z, int iter, int max );
...
int main( int argc, char *argv[] )
{
  ...
  if (pthread_mutex_init(&lock, NULL) != 0)
	...
	while((c = getopt(argc,argv,"x:y:s:W:H:m:o:k:n:h"))!=-1) {
		switch(c) {
			case 'x':
				xcenter = atof(optarg);
				break;
  ...
  for(int i = 0; i < thread_count; i++){
    ...
    //passing functions into threads
    pthread_create(&tid, &attr, compute_image, l);
    mythread[i] = tid;
  }
...
void * compute_image(void * l) {
	//declares viariables
	struct bitmap *bm;
	double xmin, xmax, ymin, ymax;
	int max,start,end,i,j;

  struct list * a; //new buffer pointer to args struct
	a = (struct list *)l;
  while(a->head != NULL){

    //locking, calling deque and assining task to each thread
    pthread_mutex_lock(&lock);
    if(a->head == NULL) {return 0; }
    int task_label = TASK_NUM;
    printf("task start %d\n", task_label);
    TASK_NUM++;
    struct task *a1 = a->head;
    if(a->head == NULL) {return 0; }
    task_dequeue(a);
...
{
  int red = z*iter/max;
	int green = 175*iter/max;
	int blue = 242*iter/max;
...
static int compute_point( double x, double y, int max)
{
  ...
  return value_to_color(x,y,z,iter,max);
